...
def get_lseg_rfr(
    ...
    overnight_rfr_lseg = [
        df.assign(
            ...
            tenor = rfr_info["tenor"].lower()
        )
        for cty_group, cty_dict in rfr_list.items()
        ...
        for df in [pd.read_csv(
            benchmark_rates_dir / cty_group / lseg_benchmark_rate_file_name.format(
                cty_group=cty_group,
                ccy_iso3=ccy_iso3,
    ...
    required_cols = {'name_simplified', 'FIXING_1', 'ccy_iso3', 'cty_group', 'Date', "rate_type", "tenor"}
    ...
    df_combined["rate"] = df_combined["rate"] / 100

    df_combined = df_combined[["date", "group", "ccy", "benchmark", "tenor", "rate_type", "rate"]]
